...
export default function AgentAnalytics() {
  ...
  const fetchData = async (d = days) => {
    setLoading(true)
    try {
      // Fetch revenue and countries in parallel
      const [res, countriesRes] = await Promise.allSettled([
        agentApi.revenueAnalytics(d),
        agentApi.countries(),
      ])
      ...
      if (countriesRes.status === 'fulfilled') {
        setCountryData((countriesRes.value as any)?.data || [])
      } else {
        setCountryData([])
      }
    } catch (err) {
  ...
  useEffect(() => {
    const unsub = onDataUpdated((data: any) => {
      if (['otps','withdrawals','users'].includes(data?.type)) fetchData(days)
    })
    return unsub
  }, [days])
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 12, color: '#64748b', marginTop: 2 }}>
              Your users' performance — real data
            </p>
          </div>
        </div>
      ...
      {/* ROW 2 — Top: Country pie + Country list (the "Who/Where" — most important info FIRST) */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
        ...
        {/* Country breakdown list — full names + percentages */}
        <div style={{ background: '#fff', borderRadius: 14, padding: '16px 20px', border: '1px solid #e2e8f0' }}>
      ...
      <div style={{ background: '#fff', borderRadius: 14, padding: '16px 20px', border: '1px solid #e2e8f0' }}>
        ...
        <p style={{ fontSize: 11, color: '#94a3b8', marginBottom: 12 }}>
          Earnings from your users' successful OTPs
        </p>
        {loading ? (
