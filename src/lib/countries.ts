...
/** Strip dial code prefix from a number string */
export function stripDialCode(num: string, dialCode: string): string {
  const stripped = dialCode.replace('+', '')
  if (num.startsWith(stripped)) return num.slice(stripped.length)
  if (num.startsWith('+' + stripped)) return num.slice(stripped.length + 1)
  return num
}
